...
Imported by app.py and all routers.
"""
import asyncio
import os
import time
...
CURALINK_URL   = os.getenv("CURALINK_URL")
USERNAME       = os.getenv("CURALINK_USERNAME")
PASSWORD       = os.getenv("CURALINK_PASSWORD")
DEFAULT_WEBAPP = os.getenv("DEFAULT_WEBAPP")

# ── HTTP client (single shared instance, SSL verification disabled for self-signed) ──
...
async def _fetch_all_series(token: str, dcm_path: str) -> list:
    ...
    while True:
        resp = await client.get(
            f"{CURALINK_URL}{dcm_path}/series?limit={limit}&offset={offset}{fields}",
            headers=headers,
        )
...
async def _fetch_all_studies_sup(token: str, dcm_path: str) -> list:
    ...
    while True:
        resp = await client.get(
            f"{CURALINK_URL}{dcm_path}/studies?limit={limit}&offset={offset}{fields}",
            headers=headers,
        )
        if resp.status_code == 200:
            page = resp.json() or []
...
# ── Hospital / institution cache ──────────────────────────────────────────────

def _build_institutions_from_series(
    series_list: list, studies_list: Optional[List[dict]] = None
) -> List[dict]:
